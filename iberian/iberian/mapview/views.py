...
class MapView(DetailView):
    ...
    def get_object(self, query_set = None):  # is dat wel goed?  
        if self.use_object: # use_object
            response = super(MapView, self).get_object(query_set)
        else:
            response = None
        return response
